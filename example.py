...
def main(**kwargs):
    ...
    use_focused_dropout = kwargs['use_focused_dropout']
    dropout = kwargs['dropout']
    par_rate = kwargs['par_rate']
    ...
    global model
    model = VGG16(use_dropout = use_dropout, use_focused_dropout = use_focused_dropout, dropout = dropout, participation_rate = par_rate)
    model.to(device)
    print('VGG model : \n', model)
    ...
    with open('./results/results.txt', 'a+') as f:
        ...
        model_string += '[dropout : ' + str(dropout) + ', use_dropout : ' + str(use_dropout) + ', use_focused_dropout : ' + str(use_focused_dropout) + '] --> acc : %.2f%%.' % (final_test_acc * 100.) + '\n'
        f.write(model_string)

    t.save(model, './results/vgg16_' + str(dropout) + '_' + str(use_dropout) + '_' + str(use_focused_dropout) + '.pth')
    ...
    plt.savefig('./results/vgg16_' + str(dropout) + '_' + str(use_dropout) + '_' + str(use_focused_dropout) + '.png')
    plt.close()
...
if __name__ == '__main__':
    ...
    """
    Usage:
    python example.py main --device='cuda' --dropout=0.5 --use_dropout=True --use_focused_dropout=False --epochs=100 --batch_size=20 --init_lr=0.1 --par_rate=0.1 --gamma=0.2 --milestones=[20,40,60,80] --weight_decay=1e-5 
    you can also import FocusedDropout module like:
    from FocusedDropout import FocusedDropout as fdo
    """
